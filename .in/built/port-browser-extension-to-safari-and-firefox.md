...
1. **Load the Firefox build**
   Open `about:debugging#/runtime/this-firefox`, choose **Load Temporary Add-on…**, select `extension/manifest.json`, and confirm the popup and overlay work.
2. **Load the Chromium build**
   Open `chrome://extensions/`, enable Developer mode, choose **Load unpacked**, select `extension/`, and confirm the popup and overlay work.
3. **Create the Safari build**
   In Xcode, create a **Safari Web Extension App**, use `extension/` as its source, run it, and confirm Safari can enable and use the extension.

Self-check: `node --check` passed for the compatibility, popup, and content scripts; manifest JSON and `git diff --check` passed.
